...
const EditJob = () => {
  ...
  useEffect(() => {
    const fetchJobDetails = async () => {
      try {
        ...
        const formattedAppDate = job.applicationDate ? new Date(job.applicationDate).toISOString().split('T')[0] : '';
        const formattedDeadline = job.deadline ? new Date(job.deadline).toISOString().split('T')[0] : '';
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      console.error(err);
      const msg = err.response?.data?.msg || 'Failed to update application';
      showToast(msg, 'error');
    } finally {
      setLoading(false);
    }
  ...
  if (fetching) {
    return (
      <div className="space-y-6 max-w-3xl mx-auto animate-fade-in">
        <div className="h-6 w-32 bg-navy-800 rounded animate-pulse"></div>
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto animate-fade-in">
      ...
      {/* Header */}
      <div>
        <h2 className="text-3xl font-extrabold text-white tracking-tight">Edit Application</h2>
        <p className="text-sm text-navy-400 mt-1">Modify application status details or add reminders</p>
      </div>
